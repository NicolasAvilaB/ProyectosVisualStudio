...
On Local Error GoTo Errlinea
  ...
  If MsgBox("Desea Eliminar El Registro Seleccionado...!", vbQuestion + vbDefaultButton2 + vbYesNo) = vbNo Then
    Exit Sub
  End If
  If EliminarTabla(txtcod, "marcas", "idmarca") > 0 Then
    frmMensajeEliminado.Show 1
    Set LIMPIAR = Me
    P_RELLENALISTAMARCAS MSHFlexGrid1, 1
  End If
Exit Sub
Errlinea:
...
If Trim(txtdes) = "" Then
MsgBox "Ingrese Marca...!", vbExclamation
Exit Sub
End If
Dim T As New ADODB.Recordset
If NUEVO = True Then
  If YaExisteMarcaN(txtdes) = True Then
    MsgBox "Ya Esta Registrado La Editorial : " + Trim$(txtdes), vbInformation
    Set T = Nothing
    Exit Sub
  End If
  T.Open "MARCAS", cn, adOpenDynamic, adLockOptimistic, adCmdTable
  T.AddNew "IDMARCA", Trim(txtcod)
Else
  If YaExisteMarcaM(txtdes, txtcod) = True Then
    MsgBox "Ya Esta Registrado La Editorial : " + Trim$(txtdes), vbInformation
    Set T = Nothing
    Exit Sub
  End If
  T.Open "SELECT * FROM MARCAs WHERE IDMARCA='" + txtcod + "'", cn, adOpenDynamic, adLockOptimistic, adCmdText
End If
  T!DESMARCA = Trim(txtdes)
  T.Update
  Set T = Nothing
  If NUEVO = True Then
...
Private Sub CmdNuevo_Click()
  ...
  habilitar
  Set LIMPIAR = Me
  txtcod.Text = GenerarCodigo(s_IDMARCAS, 5)
  txtdes.SetFocus
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
  If V_PRODUCTOS = True Then
    x = FrmProductos.cmbmarca
    P_RellenaCombo "MAR", FrmProductos.cmbmarca
    FrmProductos.cmbmarca = x
  End If
End Sub
...
If MSHFlexGrid1.TextMatrix(1, 1) = "" Then Exit Sub
If V_PRODUCTOS = True Then
  FrmProductos.cmbmarca = MSHFlexGrid1.TextMatrix(MSHFlexGrid1.RowSel, 2)
  Unload Me
End If
If V_FrmCatalogodeProductos = True Then
  FrmCatalogodeProductos.txtmarca = MSHFlexGrid1.TextMatrix(MSHFlexGrid1.RowSel, 2)
  Unload Me
End If
If V_KARDEX = True Then
  FrmKardex.txtmarca = MSHFlexGrid1.TextMatrix(MSHFlexGrid1.RowSel, 2)
  Unload Me
End If
End Sub
...
Private Sub txtbuscar_Change()
  P_RELLENALISTAMARCAS MSHFlexGrid1, 2, Trim$(txtbuscar)
End Sub
